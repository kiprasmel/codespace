...
- an org/stack-common `remote-bootstrap.sh` at `{{CONFIG_DIR}}/.codespace/`
  ...
  per-repo jobs — the only safe place for apt/NodeSource/pipx/corepack),
...
2. **Reproduce that setup for the sandbox distro** (Debian bookworm by default):
   translate any mac-only `setup-system` layer (brew/pyenv/pipx) into the distro's
   apt/NodeSource/pipx equivalent, using the repo's OWN pins (`Dockerfile*` apt
   list, `.python-version`, `.nvmrc`/`packageManager`, `Brewfile`). Put runtimes
   plainly on `PATH` — **no per-project runtime managers (mise/asdf/nvm)**.
...
4. Make `post-create` install deps via the repo's OWN command, identically local +
   remote; on the mac, if a runtime is missing, **instruct and stop** — never
   auto-install.
