...
sub-commands:
  ...
  sync        [branch|path] [-r [host]] [--commit [-m msg] | --uncommitted [--once]]
                                - mirror a local codespace to a remote ssh host,
                                  re-runnably, at commit granularity. commits
                                  integrate both ways (local is main on divergence);
                                  uncommitted changes are resolved up front.
